...
public class EarthquakeCityMap extends PApplet {
	...
	private void printQuakes() {
		...
		for (Marker country : countryMarkers) {
			for (PointFeature quake : earthquakes) {
				if (isInCountry(quake, country)) {
					int count = counts.getOrDefault(country.getProperty("name").toString(), 0) + 1;
					counts.put(country.getProperty("name").toString(), count);
					onLand++;
				}
			}
		}
		...
		int ocean = earthquakes.size() - onLand;
